...
manifest at [`talos-k8s/corral-web.yaml`](talos-k8s/corral-web.yaml)).
Code: [`tailvm-go/pkg/web/`](tailvm-go/pkg/web/), command
[`tailvm-go/cmd/web.go`](tailvm-go/cmd/web.go).
...
2. **On-cluster** — `ghcr.io/hanthor/corral-web` image (alpine + corral +
   ...
   pushed by CI (`.github/workflows/corral.yml`).
